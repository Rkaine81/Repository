function SCCMPurgeList {
    ...
    $ActiveDirs = $SCCMClient.GetCacheInfo().GetCacheElements() | ForEach-Object { Write-Output $_.Location } $MiscDirs = (Get-ChildItem -Path $SCCMCacheDir | Where-Object { (($_.PsIsContainer -eq $true) -and ($_.FullName -notin $ActiveDirs)) }).count 
    # Add Old App & Misc Directories 
    $PurgeCount = $PurgeCount + $MiscDirs 
    # Return Number Of Applications To Purge Return 
    $PurgeCount 
} 
SCCMPurgeList
