...
$flashPaths = "C:\Windows\SysWOW64\Macromed\Flash",
"C:\Windows\System32\Macromed\Flash",
"$env:APPDATA\Adobe\Flash Player",
"$env:APPDATA\Macromedia\Flash Player"

$flashKeys = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Adobe Flash Player NPAPI",
...
write-log "Beginning Folder and Registry Cleanup" $rlogFile

foreach ($flashKey in $flashKeys) {
    Remove-Item -Path $flashKey -Force -Verbose
}

foreach ($flashPath in $flashPaths) {
    Remove-Item -Path $flashPath -Force -Recurse
}